...
def _mutant_payload(m: dict) -> dict:
    """Serialize a single mutant for frontend consumption."""
    return {
        "id":           str(m.get("id", "")),
        "file":         m.get("file", ""),
        "line":         m.get("line", 0),
        ...
        "status":       m.get("status", ""),
        "mutatorName":  m.get("mutatorName", m.get("mutation_type", "")),
        "risk_score":   m.get("risk_score", 0.5),
    }
...
async def run_pipeline(job_id: str, repo: RepoInput, queue: asyncio.Queue):
    ...
    try:
        ...
        await emit(queue, JobStatus.ANALYZING, "Cloning and analyzing repository...",
                   event_route="stage_update", data={"status": "active"})
        ...
        # ── DEPLOYING ──────────────────────────────────────────────────────────
        await emit(queue, JobStatus.DEPLOYING, "Planning deployment strategy...",
                   event_route="stage_update", data={"status": "active"})
        ...
        await emit(queue, JobStatus.DEPLOYING, "Vercel deployment complete",
                   {"url": deploy_result.get("url"), "live": deploy_result.get("success"),
        ...
        if cached_mutants:
            ...
            await emit(queue, JobStatus.MUTATING, "Running mutation engine (Stryker)...",
                       event_route="stage_update", data={"status": "active"})
        ...
        await emit(queue, JobStatus.MUTATING, "Score update",
                   {"before": round(initial_score * 100), "after": round(initial_score * 100)},
                   event_route="mutation_score")

        # ── MUTATING complete — include full mutant list for Mutant Inspector ──
        await emit(queue, JobStatus.MUTATING, f"Mutation complete. Total mutants: {len(mutants)}", {
            "initial_score":  round(initial_score * 100, 1),
            "total_mutants":  len(mutants),
            "survived":       len([m for m in mutants if m["status"].lower() == "survived"]),
            "killed":         len([m for m in mutants if m["status"].lower() == "killed"]),
            "mutants":        [_mutant_payload(m) for m in mutants],
        })
        ...
        survived = rank_mutants(
            [m for m in mutants if m["status"].lower() == "survived"],
            deploy_result["repo_path"]
        )

        # Emit ranking stage so frontend shows a timestamp (not "---")
        await emit(queue, "ranking_mutants", f"Ranking {len(survived)} mutants by risk score...", {
            "count":    len(survived),
            "top_risk": survived[0].get("risk_score", 0) if survived else 0,
        })
        await emit(queue, "ranking_mutants", "Mutants ranked", {
            "mutants": [_mutant_payload(m) for m in survived],
        })

        # ── GENERATING_TESTS ───────────────────────────────────────────────────
        await emit(queue, JobStatus.GENERATING_TESTS,
                   f"Generating tests for {len(survived)} surviving mutants...",
                   event_route="stage_update", data={"status": "active"})
        ...
        await emit(queue, JobStatus.GENERATING_TESTS,
                   f"Jest run complete: {generated_tests[0].get('passed',0) if generated_tests else 0} passed, "
                   f"{generated_tests[0].get('failed',0) if generated_tests else 0} failed",
                   event_route="log")
        ...
        # ── VERIFYING ──────────────────────────────────────────────────────────
        await emit(queue, JobStatus.VERIFYING, "Running verification agents in parallel...",
                   event_route="stage_update", data={"status": "active"})
        ...
        # Emit AI analysis as a dedicated event so the dashboard can display it
        if verified_tests:
            vt = verified_tests[0]
            await emit(queue, JobStatus.VERIFYING, "AI Analysis ready", {
                "ai_analysis":  vt.get("ai_analysis", ""),
                "health_score": vt.get("health_score", "?/10"),
                "tests_run":    vt.get("tests_run", 0),
                "passed":       vt.get("passed", 0),
                "failed":       vt.get("failed", 0),
            }, event_route="ai_analysis")

        await emit(queue, JobStatus.VERIFYING,
                   f"Analysis complete — Health Score: {verified_tests[0].get('health_score','?/10') if verified_tests else '?'}",
                   event_route="log")
        ...
        # ── FIXING / final mutation run ────────────────────────────────────────
        await emit(queue, JobStatus.FIXING, "Fixing phase started",
                   event_route="stage_update", data={"status": "active"})
        ...
        await emit(queue, JobStatus.FIXING, "Score update",
                   {"before": round(initial_score * 100), "after": round(final_score * 100)},
                   event_route="mutation_score")
        ...
        # Emit scoring_reliability stage so frontend shows a timestamp
        await emit(queue, "scoring_reliability", "Calculating reliability score...", score_breakdown)
        await emit(queue, "scoring_reliability", "Reliability score calculated", score_breakdown)
        ...
        report = {
            ...
            **score_breakdown,
            ...
            "total_mutants":         len(mutants),
            "survived_before":       len([m for m in mutants if m["status"].lower() == "survived"]),
            "survived_after":        len([m for m in final_mutation.get("mutants", []) if m["status"].lower() == "survived"]),
            "tests_generated":       len(generated_tests),
            "tests_verified":        len(valid_tests),
            "vercel_project":        deploy_result.get("vercel_project", ""),
        }
        ...
        await emit(queue, JobStatus.COMPLETE, "Pipeline finished.",
                   {"report": report, **score_breakdown}, event_route="complete")
